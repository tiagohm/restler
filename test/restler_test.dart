...
void main() {
  ...
  ImportExportData restler;
  final postmanFile = File('./test/assets/postman.json');

  RequestEntity obtainRequestByName(String name) {
    return restler.requests.where((item) => item.name == name).first?.request;
  }

  setUp(() {
    // Lê o arquivo Postman.
    ...
    // Exporta para Restler.
    final restlerText =
        Exporter.restler().export(postman, password: '12345678');

    // Importa o Restler.
    restler =
        Importer.restler().importFromText(restlerText, password: '12345678');
  });
